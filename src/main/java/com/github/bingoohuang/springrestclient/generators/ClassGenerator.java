...
public class ClassGenerator<T> {
    ...
    private byte[] createImplClassBytes() {
        ...
        String classRequestMapping = getBaseUrl() + getClassRequestMapping();
    ...
    private String getBaseUrl() {
        ...
        if (!Strings.isNullOrEmpty(baseUrl)) return baseUrl;
        ...
        try {
            BaseUrlProvider provider = providerClass.newInstance();
            baseUrl = provider.getBaseUrl(restClientClass);
        } catch (Exception e) {
            throw new RuntimeException("base url configuration error for api " + restClientClass, e);
        }

        if (Strings.isNullOrEmpty(baseUrl)) {
            throw new RuntimeException("base url cannot be null generated by provider " +  providerClass);
        }
        return baseUrl;
    }
